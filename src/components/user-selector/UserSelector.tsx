...
const UserSelector = ({onUserSelect, selectedUsers = []}: UserSelectorProps) => {
   ...
   useEffect(() => {
      const fetchUsers = async () => {
         try {
            ...
            const usersCollection = collection(fireDataBase, "tenants");
            ...
            {showDropdown && (
               <div className="z-10 absolute bg-background shadow-lg mt-1 border border-input rounded-md w-full max-h-60 overflow-y-auto">
                  {loading ? (
                     ...
                     filteredUsers.map(user => (
                        <div
                           ...
                           <Avatar className="mr-2 w-8 h-8">
                              {user.photoURL ? (
                                 <img src={user.photoURL} alt={`${user.firstName} ${user.lastName}` || "User"} />
                              ) : (
                           ...
                           <div className="flex-grow">
                              <p className="font-medium text-sm">{`${user.firstName} ${user.lastName}` || "Unnamed User"}</p>
                     ...
                     <div className="p-2 text-muted-foreground text-center">{searchQuery ? "No users found" : "No users available"}</div>
                  )}
               </div>
            )}
         </div>
               ...
               <div className="flex flex-wrap gap-2">
                  {selectedUsers.map(user => (
                     <Badge key={user.uid} variant="secondary" className="flex items-center gap-1">
                        ...
                        <X className="w-3 h-3 cursor-pointer" onClick={() => handleUserRemove(user.uid)} />
                     </Badge>
                  ))}
               </div>
            </div>
