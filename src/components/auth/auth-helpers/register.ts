...
const userRegistration = async (setIsLoading, formData, setTab) => {
  try {
    ...
    if (!formData.email || !formData.password || !formData.userType || !formData.name) {
    ...
    if (currentUser) {
      ...
      if (!docSnap.exists()) {
        ...
        await setDoc(userDocRef, {
          ...
          status: 'active',
          ...(formData.userType === 'landlord' ? { propertyRefs: [], tenantRefs: [] } : {}),
          ...(formData.userType === 'tenant' ? { propertyRef: null } : {}),
        });
