...
const PropertyOverview = ({ properties = defaultProperties }: PropertyOverviewProps) => {
  ...
  const handlePropertyUpdated = updatedProperty => {
    ...
    setUserProperties(prevProperties =>
      prevProperties.map(prop => (prop.uid === updatedProperty.uid ? updatedProperty : prop))
    );
  };
  useEffect(() => {
  ...
  const filteredProperties =
    activeTab === 'all'
      ? displayProperties
      : displayProperties.filter(property => property.propertyType === activeTab);
  ...
  const occupancyRate =
    totalUnits > 0
      ? Math.round(
          (displayProperties.reduce((sum, property) => sum + (property.occupiedUnits || 0), 0) /
            totalUnits) *
            100
        )
      : 0;
  ...
  return (
    ...
    >
      <DialogContent className="max-w-[95vw] sm:max-w-[525px] max-h-[90vh] overflow-y-auto">
        ...
        {error && (
          <div className="bg-destructive/10 p-3 rounded-md text-destructive text-sm">{error}</div>
        )}
